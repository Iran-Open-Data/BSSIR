...
from .core import data_cleaner, resource_handler
from . import (
    core,
    data_engine,
    utils,
...
class API:
    def __init__(self, context: Context):
        ...
        self.utils = utils.Utils(config=context.config)
    ...
    def add_classification(self, table: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        return core.add_classification(
            table,
            context=self.context,
            **kwargs,
        )
    ...
    def add_weight(self, table: pd.DataFrame) -> pd.DataFrame:
        ...
        years = (
            core.extract_column(table, self.context.config.standard_columns.year).unique().tolist()
        )
