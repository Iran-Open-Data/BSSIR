...
def load_context(
    *,
    ...
    return Context(config=config, metadata=metadata)
...
# from .transformers import update_dict


# BASE_PACKAGE_DIRECTORY = Path(__file__).parents[1]
# ROOT_DIRECTORY = Path().absolute()

# Years = int | Iterable[int] | str | Literal["all", "last"]
...
#     @staticmethod
#     def industries_interpreter(yaml_text: str, context: dict) -> str:
#         yaml_text = Metadata.anchor_handler(yaml_text, context)
#         return yaml_text

#     @staticmethod
#     def occupations_interpreter(yaml_text: str, context: dict) -> str:
#         yaml_text = Metadata.anchor_handler(yaml_text, context)
#         return yaml_text
...
#         self.settings = self.get_base_config()
#         self.settings = update_dict(
#             self.settings, self.get_package_config(package_path)
#         )
#         self.settings = update_dict(self.settings, self.get_root_config())
