...
from .. import metadata_reader as meta
...
def weighted_average(
    ...
    """
    weight_col = meta.defaults.columns.weight if weight_col is None else weight_col
    ...
    if columns is None:
        columns = [
            ...
            if col not in meta.defaults.columns.groupby
            if col not in [meta.defaults.columns.id, weight_col, "index"]
        ]
...
def average_table(
    ...
    if groupby is None:
        groupby = [col for col in table.columns if col in meta.defaults.columns.groupby]
